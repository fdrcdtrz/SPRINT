...
if __name__ == '__main__':
    ...
    for num_services in num_services_list:
        ...
        flops_values = np.random.randint(1000, 1000000, num_services)
        ...
        N0 = 10e-10
        lmbd_values = np.random.randint(100, 200, num_resources)
        ...
        # results_dir)
        #
        # Q_I = optimize_kpi(services, resources, normalized_kpi, normalized_kvi, weighted_sum_kpi,
        # weighted_sum_kvi, results_dir)
        #
        # V_N = v_nadir(services, resources, normalized_kpi, normalized_kvi,
        # weighted_sum_kpi, weighted_sum_kvi, Q_I, results_dir)
        #
        # Q_N = q_nadir(services, resources, normalized_kpi,
        # normalized_kvi, weighted_sum_kpi, weighted_sum_kvi, V_I, results_dir) #
        #
        # pareto_solutions_exact =
        # epsilon_constraint_exact(services, resources, normalized_kpi, normalized_kvi, weighted_sum_kpi,
        # weighted_sum_kvi, Q_N, Q_I, delta=delta, results_dir=results_dir)
        #
        ...
        #
        # lambda_ = np.full((len(services), len(resources)), 0)  # per ogni (servizio, risorsa)
        # mu_ = np.full((len(services), len(resources)), 0)  # per ogni (servizio, risorsa)
        # nu_ = np.full(len(services), 0)  # per ogni servizio
        #
        # total_value, assignments = multi_knapsack_dp(services, resources, weighted_sum_kpi, weighted_sum_kvi, lambda_,
        #                                              mu_, nu_)
        ...
        #     services, resources, assignments, weighted_sum_kpi, weighted_sum_kvi, min_kpi, min_kvi
        # )

        # # Calcola il valore della soluzione riparata
        # total_value_repaired = compute_total_value(
        #     services, resources, item_assignment_repaired, weighted_sum_kpi, weighted_sum_kvi
        # )
        #
        # print("Valore totale dopo riparazione:", total_value_repaired)
        ...
        #     services, resources, item_assignment_repaired, weighted_sum_kpi, weighted_sum_kvi,
        #     lambda_, mu_, nu_, UB, LB, total_value, total_value_repaired
        # )
        #
        # print("Lambda aggiornato:\n", lambda_)
        # print("Mu aggiornato:\n", mu_)
        # print("Nu aggiornato:\n", nu_)
        ...
        for k in range(max_iterations):
            ...
            # Riparazione
            item_assignment_repaired = repair_solution(
                services, resources, item_assignment, weighted_sum_kpi, weighted_sum_kvi, min_kpi, min_kvi
            )
